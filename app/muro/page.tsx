...
export default async function MuroPage() {
  ...
  return (
    <main className="relative min-h-dvh overflow-hidden bg-[#0A0A0A] text-white">
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-50 flex items-start justify-between gap-6 bg-gradient-to-b from-[#0A0A0A]/92 via-[#0A0A0A]/55 to-transparent p-5 pb-20 sm:p-8 sm:pb-24">
        <h1
          className="max-w-[calc(100%-8rem)] text-4xl font-black uppercase leading-[0.98] tracking-[0.08em] text-[#FF323C] [font-variant-ligatures:none] [text-shadow:4px_4px_0_#0A0A0A] sm:max-w-[calc(100%-11rem)] sm:text-6xl lg:text-7xl"
          style={{
            fontFamily:
      ...
      />
      <footer className="pointer-events-auto absolute inset-x-0 bottom-0 z-50 border-t border-white/10 bg-[#0A0A0A]/75 px-5 py-4 text-center font-mono text-xs leading-6 text-zinc-400 backdrop-blur-sm sm:text-sm">
        Creado por{" "}
        <a
